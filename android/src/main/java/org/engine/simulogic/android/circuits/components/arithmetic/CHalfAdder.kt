...
class CHalfAdder(x:Float, y:Float, rotationDirection:Int, private val scene: PlayGroundScene) :CNode(){
    ...
    init {
        ...
        val spriteRegion = textureAtlas.findRegion("HALF-ADDER")
        type = CTypes.ARITHMETIC_HALF_ADDER
    ...
    override fun clone():CNode {
        return CHalfAdder(getPosition().x,getPosition().y, rotationDirection, scene )
    }
