...
class CDotMatrixDisplay(x:Float, y:Float,rotationDirection:Int, private val scene: PlayGroundScene) :CNode(){

    private val lines = mutableListOf<CLine>()
    private val matrixArray = mutableListOf<MutableList<CMatrixPixel>>()
    ...
    override fun clone():CNode {
        return CDotMatrixDisplay(getPosition().x,getPosition().y, rotationDirection, scene )
    }
