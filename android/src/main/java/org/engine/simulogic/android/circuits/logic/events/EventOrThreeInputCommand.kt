...
class EventOrThreeInputCommand(private val position: Vector2, private val connection: Connection,
                     private val commandHistory: CommandHistory,
                     private val scene: PlayGroundScene) : Command() {

    override fun execute() {
        ListNode(COrThreeInput(position.x, position.y, scene)).also { node ->
