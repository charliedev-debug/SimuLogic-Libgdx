...
class AutoSave private constructor(private val projectOptions: ProjectOptions, private val connection: Connection) {
   ...
   private val dto = DataTransferObject()
    ...
    companion object {
        ...
        fun initialize(projectOptions: ProjectOptions, connection: Connection){
            instance = AutoSave(projectOptions, connection)
        }
    }

    fun forceSave(){
        if(dataChanged){
            dto.writeData(projectOptions, connection)
            dataChanged = false
        }
    }
