...
open class CSprite (x:Float, y:Float, width:Float, height:Float, textureString:String, var color: Color, private val scene: PlayGroundScene) : CNode(){
    init {

        val textureAtlas = scene.assetManager.get("${EnvironmentTheme.name}.atlas", TextureAtlas::class.java)
        val spriteRegion = textureAtlas.findRegion(textureString)
