...
class CNorThreeInput(x:Float, y:Float,rotationDirection:Int = ROTATE_RIGHT, private val scene: PlayGroundScene) :CNode(){
    ...
    init {
        ...
        val spriteRegion = textureAtlas.findRegion("NOR-DARK")
        type = CTypes.NOR_THREE_INPUT
        this.rotationDirection = rotationDirection
        sprite = Sprite(spriteRegion).apply {
            setOrigin(x , y)
            setSize(CDefaults.gateWidth * 1.5f , CDefaults.gateWidth * 1.5f)
            setOriginCenter()
            ...
            setPosition(x - CDefaults.gateWidth * 1.5f / 2f,y - CDefaults.gateWidth * 1.5f / 2f)
        }

        signals.add(CSignal(x + sprite.width * 0.8125f, y ,CTypes.SIGNAL_OUT,0, scene))
        signals.add(CSignal(x - sprite.width * 0.8125f, y + sprite.height / 2f - sprite.height  ,CTypes.SIGNAL_IN, 1, scene))
        signals.add(CSignal(x - sprite.width * 0.8125f, y - sprite.height / 2f + sprite.height  ,CTypes.SIGNAL_IN, 2, scene))
        signals.add(CSignal(x - sprite.width * 0.8125f, y ,CTypes.SIGNAL_IN, 3, scene))
        signals.forEach {
            attachChild(it)
        }
    ...
    override fun execute() {
        ...
        output.value = if(inputA.value.or(inputB.value).or(inputC.value) == 1) 0 else 1
    }
    ...
    override fun clone():CNode {
        return CNorThreeInput(getPosition().x,getPosition().y,rotationDirection, scene )
    }
