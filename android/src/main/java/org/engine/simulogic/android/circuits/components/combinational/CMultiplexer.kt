...
class CMultiplexer(x:Float, y:Float,rotationDirection:Int, private val title:String, private val font: BitmapFont, private val connection: Connection, private val scene: PlayGroundScene) :CNode(){
    ...
    init {
        ...
        val maxWidth = inputDataCount * CDefaults.muxerSignalSpacing
        val multiplexerInputDataWidth = maxWidth
        val multiplexerSelectorHeight = inputSelectorCount * CDefaults.muxerSignalSpacing
        this.rotationDirection = rotationDirection
        sprite = Sprite(spriteRegion).apply {
            setOrigin(x , y)
            setSize(multiplexerInputDataWidth, multiplexerSelectorHeight)
            setOriginCenter()
            ...
            setPosition(x - multiplexerInputDataWidth / 2f,y - multiplexerSelectorHeight / 2f)
        }
        ...
        for( i in 0 until inputDataCount){
            CSignal(getCenter().x + i * sprite.width / inputDataCount + offsetLeft / 2f,
                getCenter().y + multiplexerSelectorHeight + CDefaults.muxerSignalSpacing, CTypes.SIGNAL_IN,i,scene).also { signal ->
                signals.add(signal)
        ...
        scene.getLayerById(LayerEnums.CONNECTION_LAYER.name).also { layer->
            // bottom line
            lines.add(CLine(getCenter().x, getCenter().y, getCenter().x + multiplexerInputDataWidth,getCenter().y,lineWidth * 2f))
            // top line
            lines.add(CLine(getCenter().x,getCenter().y + multiplexerSelectorHeight,getCenter().x + multiplexerInputDataWidth,getCenter().y + multiplexerSelectorHeight,lineWidth * 2f))
            // left line
            lines.add(CLine(getCenter().x ,getCenter().y,getCenter().x , getCenter().y + multiplexerSelectorHeight,lineWidth * 2f))
            // right line
            lines.add(CLine(getCenter().x + multiplexerInputDataWidth,getCenter().y,getCenter().x + multiplexerInputDataWidth, getCenter().y + multiplexerSelectorHeight,lineWidth * 2f))
    ...
    override fun update() {
        ...
        when(rotationDirection){
            ROTATE_RIGHT->{
                ...
                for( i in 0 until inputDataCount){
                    val signal = signals[i]
                    signal.updatePosition(getCenter().x + i * CDefaults.muxerSignalSpacing + offsetLeft / 2f,getCenter().y + multiplexerSelectorHeight + CDefaults.muxerSignalSpacing)
        ...
        // top line
        lines[1].updatePosition(getCenter().x,getCenter().y + multiplexerSelectorHeight,
            getCenter().x + maxWidth,getCenter().y + multiplexerSelectorHeight)
        // left line
